...
def generate_video(network_pkl, seed, truncation_psi, num_images):
    ...
    for image_idx, z in enumerate(np.linspace(z_start, z_stop, num_images)):
        ...
        PIL.Image.fromarray(im, 'RGB').save(im_path + '/image%04d.png' %image_idx)


def generate_grid_video(network_pkl, seed, truncation_psi, num_images, grid_shape=(2, 17)):
    print('Loading networks from "%s"...' % network_pkl)
    _G, _D, Gs = pretrained_networks.load_networks(network_pkl)
    noise_vars = [var for name, var in Gs.components.synthesis.vars.items() if name.startswith('noise')]
    # noise_vars = noise_vars[2:]
    ...
    im_path = 'video/grid_seed%04d' %seed
    os.makedirs(im_path, exist_ok=True)
    rnd = np.random.RandomState(seed)
    latent_shape, label_shape = Gs.input_shapes
    labels = np.zeros([1, *label_shape[1:]])
    ...
    for image_idx, z in enumerate(np.linspace(z_start, z_stop, num_images)):
        ...
        for i in range(label_shape[1]):
            labels = np.zeros([1, *label_shape[1:]])
            labels[0, i] = 1.
            images = Gs.run(z, labels, **Gs_kwargs) # [minibatch, height, width, channel]
            # im = PIL.Image.fromarray(images[0], 'RGB')
            im_list.append(images[0])
        im = montage(arr_in=np.array(im_list), grid_shape=grid_shape, multichannel=True)
        PIL.Image.fromarray(im, 'RGB').save(im_path + '/image%04d.png' %image_idx)
...
'''
# ipython run_generator.py -- style-mixing-example --network='/home/romain/win_desk/stylegan2/results/00098-stylegan2-feuilles1k-1gpu-config-e/network-snapshot-000640.pkl' --row-seeds=851,442,838,56 --col-seeds=155,67,748,356,886,310,8024,12018 --truncation-psi=0.85
# ipython run_generator.py -- generate-images --network=/home/romain/win_desk/stylegan2/results/00021-stylegan2-feuilles2k-1gpu-config-f/network-snapshot-000360.pkl --seeds=6000-6025 --truncation-psi=0.5
#----------------------------------------------------------------------------
...
Run 'python %(prog)s <subcommand> --help' for subcommand help.''',
    ...
    parser_generate_video = subparsers.add_parser('generate-video', help='Generate video')
    parser_generate_video.add_argument('--network', help='Network pickle filename', dest='network_pkl', required=True)
    parser_generate_video.add_argument('--seed', type=int, help='A random seed', required=True)
    ...
    parser_generate_video.add_argument('--num-images', type=int, help='number of generated images (default: %(default)s', default=100)
