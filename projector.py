...
class Projector:
    def __init__(self):
        ...
        self.initial_learning_rate      = 0.25
    ...
    def set_network(self, Gs, minibatch_size=1):
        ...
        if sh[2] > 256:
            factor = sh[2] // 256
            # proc_images_expr = tf.reduce_mean(tf.reshape(proc_images_expr, [-1, sh[1], sh[2] // factor, factor, sh[2] // factor // 2, factor // 2]), axis=[3,5])
            proc_images_expr = tf.reshape(tf.transpose(tf.image.resize(tf.transpose(proc_images_expr[0]), (256, 256))), [-1, 3, 256, 256])
        ...
        for v in self._noise_vars:
            ...
            while True:
                ...
                v = tf.reshape(v, [1, 1, sz//2, 2, sh//2, 2]) # Downscale
    ...
    def run(self, target_images, dlatent_start=None):
        # Run to completion.
        self.start(target_images, dlatent_start=dlatent_start)
    ...
    def start(self, target_images, dlatent_start=None):
        ...
        if sh[2] > self._target_images_var.shape[2]:
            ...
            target_images = resize(target_images[0].T, (256, 256)).T[np.newaxis]
    ...
    def step(self):
        ...
        if self._cur_step == 0:
            self._info('Running...')
            self._info('Current Step, Distance, Loss')
