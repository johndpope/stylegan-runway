...
class TFRecordExporter:
    ...
    def add_image(self, img):
        ...
        if self.shape is None:
            ...
            assert self.shape[1] == 2*self.shape[2]
...
def create_from_images(tfrecord_dir, image_dir, shuffle,
                       n_labels=None, size=(512, 1024)): # WxH (1024, 2048)
    # from IPython import embed; embed()
    print('Loading images from "%s"' % image_dir)
    image_filenames = sorted(glob.glob(os.path.join(image_dir, '*')))
    if len(image_filenames) == 0:
    ...
    n_side, n_tree = len(side_enc.classes_), len(tree_enc.classes_)
    n_labels = n_side * n_tree
    print('found %i labels' %n_labels)
    ...
    with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
        order = tfr.choose_shuffled_order() if shuffle else np.arange(len(image_filenames))
        onehot = np.zeros((len(image_filenames), n_labels), dtype=np.float32)
        for idx in range(order.size):
            image_filename = image_filenames[order[idx]]
            im = PIL.Image.open(image_filename)
            img = np.asarray(im.resize(size))
            if img.size != 1:
                if channels == 1:
                    img = img[np.newaxis, :, :] # HW => CHW
                elif img.size != 1:
            ...
            # process label
            tokens = os.path.basename(image_filename).split('_')
            label = np.zeros([n_side, n_tree])
            # parse leaf side info
            eside = side_enc.transform([tokens[0].lower()])
            # parse leaf tree information
            etree = tree_enc.transform([' '.join(tokens[1:-1]).lower()])
            label[eside, etree] = 1
            onehot[order[idx], :] = label.reshape(n_labels)
