...
def G_synthesis_stylegan_revised(
    ...
    resolution          = 2048,         # Output resolution.
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    # Early layers.
    with tf.variable_scope('4x2'):
        with tf.variable_scope('Const'):
            x = tf.get_variable('const', shape=[1, nf(1), 4, 2], initializer=tf.initializers.random_normal())
    ...
    def block(res, x): # res = 3..resolution_log2
        im_width = (2**res) // 2
        im_height = (2**res)
        with tf.variable_scope('%dx%d' % (im_height, im_width)):
...
def D_stylegan(
    ...
    resolution          = 2048,         # Input resolution. Overridden based on dataset.
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    images_in.set_shape([None, num_channels, resolution, resolution//2])
    labels_in.set_shape([None, label_size])
...
def D_stylegan2(
    ...
    nonlinearity        = 'lrelu',      # Activation function: 'relu', 'lrelu', etc.
    mbstd_group_size    = 1,            # Group size for the minibatch standard deviation layer, 0 = disable.
    mbstd_num_features  = 1,            # Number of features for the minibatch standard deviation layer.
    dtype               = 'float32',    # Data type to use for activations and outputs.
    resample_kernel     = [1,3,3,1],    # Low-pass filter to apply when resampling activations. None = no filtering.
    **_kwargs):                         # Ignore unrecognized keyword args.
